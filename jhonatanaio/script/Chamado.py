...
class Support_Window(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def populateMotivos(self):
        ...
        motivos2=motivos1
        motivos3=["Escolha o Motivo","FI sem assinatura","Problemas All In One (posto ao lado)","Falha Nascimento do terminal","******* Test não abre","Lentidão no ******* Test","Reprovação de terminal","Terminais não abrem tela do Cliente","Todos Genisys Offline","Todos terminais com falha de gravação","Um Genisys Offline","Um terminal com falha de gravação","Micro Travado / Reiniciando","Outro"]
        motivos4=["Escolha o Motivo","FI sem assinatura","Problemas All In One (posto ao lado)","Falha Nascimento do terminal","******* Test não abre","Lentidão no ******* Test","Reprovação de terminal","Terminais não abrem tela do Cliente","Falha no SAFF Extractor","Micro Travado / Reiniciando","Outro"]
        motivos5=["Escolha o Motivo","FI sem assinatura","Problemas All In One (posto ao lado)","Outro"]
        motivos6=["Escolha o Motivo","FI sem assinatura","Problemas All In One (posto ao lado)","******* Test não abre","Lentidão no ******* Test","Troca de Etiqueta / Ribbon","Micro Travado / Reiniciando","Um terminal com falha agregação","Todos Terminais com falha de agregação","Falha Pick To Light","Falha na Rotuladora","Etiqueta não gerada - Zebra","Regerar Etiqueta","Outro"]
        motivos7=["Escolha o Motivo","FI sem assinatura","Problemas All In One (posto ao lado)","******* Test não abre","Lentidão no ******* Test","Reprovação de terminal","Troca de Etiqueta / Ribbon","Micro Travado / Reiniciando","Falha na Rotuladora","Regerar Etiqueta","Etiqueta trocada / Ausente","Reteste camera","Falha jiga / sensor","Outro"]
        motivos8=["Escolha o Motivo","FI sem assinatura","Problemas All In One (posto ao lado)","******* Test não abre","Lentidão no ******* Test","Micro Travado / Reiniciando","Falha no braço da balança","Camera Balança não lê","Uma caixa com peso fora do especificado","Todas caixas - peso fora do especificado","Um terminal não embala","Todos terminais não embalam","MES não abre","Caixa no BIN","Outro"]
        motivos9=["Escolha o Motivo","FI sem assinatura","Problemas All In One (posto ao lado)","******* Test não abre","Micro Travado / Reiniciando","Problema no Relatorio de Pallet / BOX","Duvidas Script de liberação de Pallet","Falha Script de liberação do Pallet","Outro"]
    ...
    def iniciarChamado(self):
        ...
        url2 = 'http://*******m0itqa01/AioWatch/Update'
    ...
    def enviaChamado(self):
        ...
        headers = {'content-type': 'application/json'}
        url = 'http://*******m0itqa01/AioWatch/Create'
    ...
    def finaliza(self):
        ...
        headers = {'content-type': 'application/json'}
        url3 = 'http://*******m0itqa01/AioWatch/Update'
...
class WatchStatus(QThread):
    ...
    def startThread(self,janelaSuporte):
        ...
        self.janelaSuporte = janelaSuporte
        self.url = "http://*******m0itqa01/AioWatch/GetById?id=" + self.janelaSuporte.requestID
