...
class MFRC522:
  ...
  def close_SPI(self):
    spi.closeSPI(self.dev_dictionary)
  ...
  #Metodo que retorna a matrícula *******
  def *******_Matricula(self):
    #Variavel de retorno
    matricula = None
    errorsMat = 0
    while (matricula == None and errorsMat <= 3):
      ...
      if status == self.MI_OK:
        #Chave para autenticacao dos crachas *******
        key = [0xA3,0x0D,0xEF,0x4F,0x61,0x1F]
        ...
        if status == self.MI_OK:
          ...
          try:
            matricula = ("%02d" % (backData[2],))+("%02d" % (backData[3],))+("%02d" % (backData[4],))+("%02d" % (backData[5],))
          except:
            e = sys.exc_info()
      ...
      if (matricula == None):
        time.sleep(0.2)
      errorsMat = errorsMat + 1
    #print("Retorno Matricula: " + matricula)
    return matricula
